...
final class RecordEntryUseCaseTests: XCTestCase {
    ...
    func testKeepsAudioWhenTranscriptionIsEmpty() async throws {
        let repository = InMemoryEntryRepository()
        let useCase = RecordEntryUseCase(entries: repository)
        ...
        XCTAssertEqual(entry?.audioFileName, "recording.m4a")
        XCTAssertEqual(repository.storage.count, 1)
    }
...
final class ReplaceEntryTranscriptUseCaseTests: XCTestCase {
    func testReplacesTranscriptAndPreservesOldReflection() async throws {
        ...
        let updated = try await useCase.execute(
            entryID: entry.id,
            transcriptText: "  Agora em português.  "
        )
    ...
    func testDoesNotEraseTranscriptWhenNewResultIsEmpty() async throws {
        let repository = InMemoryEntryRepository()
        let entry = makeEntry()
        try await repository.save(entry)
        let useCase = ReplaceEntryTranscriptUseCase(entries: repository)

        let updated = try await useCase.execute(entryID: entry.id, transcriptText: " ")
